...
3. **在线考试**：倒计时、题目导航快速跳转、标记稍后作答、最后 5 分钟提醒、时间到自动提交。
   - **个别考生延时**：已发布试卷可给个别考生登记补时（5~60 分钟 + 原因，交卷前登记）；一名考生一场考试仅一条有效记录，开考前可撤销、开考后冻结，重复/并发/越界请求拒绝；开考与收卷按个人截止时间，其他考生不受影响，试卷总分与标准答案不变。
...
| GET | /exams/:id/extensions | 教师/管理员 | 某考试全部个别补时记录 |
| GET | /exam-extension-students | 教师/管理员 | 补时登记用学生选项（支持 keyword） |
| POST | /exams/:id/extensions | 教师/管理员 | 给个别考生登记补时（5~60 分钟 + 原因，交卷前） |
| DELETE | /exams/extensions/:extId | 教师/管理员 | 撤销补时（仅开考前，开考后冻结） |
| GET | /exams/:id/my-extension | 学生 | 查询本人补时分钟与个人截止时间 |
