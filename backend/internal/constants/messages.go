...
const (
	...
	MsgExtensionGrantSuccess  = "个别考生补时登记成功"
	...
	MsgExtensionExists     = "补时模块：考生 student_id=%s 在考试 exam_id=%s 已有一条有效补时记录"
	MsgExtensionMinutes    = "补时模块：补时分钟字段 extra_minutes=%d 越界，必须在 %d~%d 分钟之间"
	MsgExtensionFrozen     = "补时模块：考试 exam_id=%s 已开考，补时记录冻结不可撤销"
	MsgExtensionExamState  = "补时模块：试卷 exam_id=%s 当前状态 %s 不允许登记补时（须已发布且未交卷）"
	MsgExtensionReason     = "补时模块：原因字段 reason 不能为空，长度限制 1~500"
	MsgExtensionStudent    = "补时模块：目标 student_id=%s 不存在或角色不是学生"
