...
type ExamRecord struct {
	...
	// 个别考生补时快照（与 exam_extensions 有效记录同步；0 表示无补时）。
	// 个人截止时间 = 开考时间 + (试卷时长 + ExtraMin) 分钟，收卷按该个人截止时间执行。
	ExtraMinutes int       `bson:"extra_minutes" json:"extra_minutes"`
	DeadlineAt   time.Time `bson:"deadline_at,omitempty" json:"deadline_at"`
